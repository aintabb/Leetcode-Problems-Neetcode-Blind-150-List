...
class ValidSudoku:
    def __init__(self) -> None:
        err_msg_invalid_valid_sudoku = (
            "Provided sudoku is not valid. Something is wrong!"
        )
        board = [
            ["5", "3", ".", ".", "7", ".", ".", ".", "."],
            ["6", ".", ".", "1", "9", "5", ".", ".", "."],
    ...
    def is_valid_sudoku(self, board: list[list[str]]) -> bool:
        ...
        for row in range(9):
            for col in range(9):
                ...
                if (
                    curr_val in rows[row]
                    or curr_val in cols[col]
                    or curr_val in grids[(row // 3, col // 3)]
                ):
                    return False

                rows[row].add(curr_val)
                cols[col].add(curr_val)
                grids[(row // 3, col // 3)].add(curr_val)
